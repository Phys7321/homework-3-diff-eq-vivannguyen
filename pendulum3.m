function [kin,pot,energy,period,t,w] = pendulum3(omega0,omega,theta0,thetad0,gamma,grph) 
...
[t,w] = ode45(@proj,tspan,r0,opts,omega0,gamma);
sol = [t,w];
...
%-------------------------------------------
%
function rdot = proj(t,r,omega0,gamma)
%    rdot = [r(2); -g/R*sin(r(1))]; now have initial omega
    rdot = [r(2); -omega0*sin(r(1))-gamma*r(2)];
end

function [value,isterminal,direction] = events(t,w,omega0,gamma)
value = int8(abs(w(1))<0.0001 & abs(w(2))<abs(w(1))*omega0);
isterminal = 1;
direction = 0;
end